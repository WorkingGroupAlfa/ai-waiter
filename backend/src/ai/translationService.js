...
export async function translateFromEnglish(textEn, targetLang) {
  ...
  const systemPrompt =
    'You are a strict translation engine for restaurant chat. Translate ALL natural-language text from English into the requested target language with no code-switching. Keep proper product/brand/cocktail names unchanged. Return ONLY the translated text.';
...
export async function translateText(text, targetLang, sourceLang = null) {
  ...
  const systemPrompt =
    'You are a strict translation engine for restaurant UI text. Translate ALL natural-language text into the target language with no mixed-language output. Keep proper product/brand/cocktail names unchanged. Preserve numbers, currency symbols, punctuation, markdown, and HTML tags exactly as they are. Return ONLY translated text.';
